...
local activeDisables = {}
local activeDisablesCount = 0
...
local function cacheActiveControls()
    activeDisables = {}
    activeDisablesCount = 0
    disableCombat = false

    if Action.controlDisables then
        disableCombat = Action.controlDisables.disableCombat or false
        for disableType, isEnabled in pairs(Action.controlDisables) do
            if isEnabled and controls[disableType] then
                local list = controls[disableType]
                for i = 1, #list do
                    activeDisablesCount = activeDisablesCount + 1
                    activeDisables[activeDisablesCount] = list[i]
                end
            end
        end
    end
    ...
    if (not isPlayerDead or action.useWhileDead) and not isDoingAction then
        ...
        CreateThread(function()
            ...
            while isDoingAction do
                ...
                for i = 1, activeDisablesCount do
                    DisableControlAction(0, activeDisables[i], true)
                end
                ...
                if action.canCancel and IsControlJustPressed(0, 200) then
                    TriggerEvent('progressbar:client:cancel')
                    wasCancelled = true
                    break
                end
                
                if not action.useWhileDead and IsEntityDead(playerPed) then
                    TriggerEvent('progressbar:client:cancel')
                    wasCancelled = true
                    break
                end
